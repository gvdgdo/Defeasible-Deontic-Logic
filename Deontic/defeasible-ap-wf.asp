...
supported(X) :- 
    ...
    constitutiveRule(R,X), not defeater(R,X), support(R,X), 
    discarded(S,Y) : opposes(X,Y), constitutiveRule(S,Y), superior(S,R).
...
defeasible(X) :- 
    definiteRefuted(X1) : opposes(X,X1); 
    constitutiveRule(R,X), not defeater(R,X), applicable(R,X),
...
defeated(R,X) :- opposes(X,X2), constitutiveRule(R,X),
    constitutiveRule(R2,X2), support(R2,X2), superior(R2,R).
